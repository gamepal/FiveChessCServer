...
#define MAX_PKG_SIZE ((1<<16) - 1)   //数据接收不超过65535位 ～64字节
#define MAX_RECV_SIZE 2047           //缓存大小 - 1
...
struct io_package {
	...
	int opt; //当前的请求的类型 IOCP_ACCPET IOCP_RECV
	int accpet_sock;
	int recved; // 收到的字节数;
...
process_ws_shake_hand(struct session* ses, struct io_package* io_data,
	...
	if (has_client_key == 0) {
		...
		int ret = WSARecv(ses->client_socket, &(io_data->wsabuffer),
			1, &dwRecv, &dwFlags,
			&(io_data->overlapped), NULL);
	...
	send(ses->client_socket, accept_buffer, strlen(accept_buffer), 0);
	ses->is_shake_hand = 1;
...
on_ws_pack_recved(struct session* s, struct io_package* io_data, int protocal_type) {
	...
	while (io_data->recved > 0) {
		...
		if (recv_ws_header(io_data->pkg, io_data->recved, &pkg_size, &header_size) != 0) { // 继续投递recv请求，知道能否接收一个数据头;
			...
			int ret = WSARecv(s->client_socket, &(io_data->wsabuffer),
				1, &dwRecv, &dwFlags,
				&(io_data->overlapped), NULL);
			break;
		}
		...
		if (io_data->recved >= pkg_size) { // 表示我们已经收到至少超过了一个包的数据；
			...
			// 0x81
			if (pkg_data[0] == 0x88) { // 对方要关闭socket Websocket客户端关闭会向服务器发送关闭消息
